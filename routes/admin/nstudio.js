...
var productUpload = upload.fields([{ name: "images[]", maxCount: 20 }]);
...
router.post("/:id/edit", productUpload, function(req, res, next) {
  ...
  NstudioProduct.findById(id, function(error, product) {
    if (!error) {
      ...
      if (data.name_cn) product.name_cn = data.name_cn;
      if (data.description_cn) product.description_cn = data.description_cn;
